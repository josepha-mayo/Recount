...
test('ASR past-tense confirmation variant still requires the exact quantity echo',()=>{let st=initial();st=apply(st,msg('Rice, 13 bags.',[w('Rice,'),w('13'),w('bags.')],0));st=apply(st,msg('Confirmed 13.',[w('Confirmed',.85),w('13',.99)],1));assert.equal(st.counts.rice.quantity,13);assert.equal(st.pending,null);});
