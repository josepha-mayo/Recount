...
def open_session(browser,width):
 ...
 page.goto(BASE+'/',wait_until='networkidle');page.locator('summary').click();page.locator('#consent').check();page.locator('#speak').check();page.locator('#listen').click();expect(page.locator('#mode')).to_have_text('AUDIO LISTENING')
 return ctx,page
try:
 for _ in range(40):
 ...
 with sync_playwright() as pw:
  ...
  for width in [1360,390]:
   ...
   assert len(r['asset_sha256'])==8 and 'PRIVATE_CSRF_SENTINEL' not in raw and 'PRIVATE_PROVIDER_SENTINEL' not in raw
